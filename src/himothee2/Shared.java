...
public class Shared {
    ...
    /** True if paintLoc is within the 5×5 SRP pattern centered at srpCenter. */
    static boolean isWithinSRPPattern(MapLocation srpCenter, MapLocation paintLoc) {
        return Math.abs(paintLoc.x - srpCenter.x) <= 2 && Math.abs(paintLoc.y - srpCenter.y) <= 2;
    }
    ...
    public static void bug2(RobotController rc, MapLocation target) throws GameActionException {
        ...
        if (!isTracing) {
            ...
            if (rc.canMove(tracingDir)) {
                rc.move(tracingDir);
                // After moving forward along the wall, rotate left to hug the wall again (left-hand rule)
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    if (rc.canMove(tracingDir)) {
                        rc.move(tracingDir);
                        // Turned right to get around obstacle; rotate left to try to hug wall next step
                        tracingDir = tracingDir.rotateLeft();
                        break;
                    }
                }
